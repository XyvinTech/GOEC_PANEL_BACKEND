...
import { updateStation } from '@/lib/features/chargingStations/stationAPI';
...
const EditStationData: React.FC<EditStationProps> = ({ stationData }) => {
    const initialStationData: Station = stationData ?? {
        // _id: '',
        stationName: '',
        goecOnly: false,
        location: {
    ...
    const [selectedState, setSelectedState] = stationData?.location.state ? useState<string>(stationData?.location.state) : useState<string>('');
    ...
    const mapIdsToOptions = useMemo(() => {
        return (ids: string[] | undefined, stations: Station[]): Items[] => {
            if (!ids) return [];
    ...
    useEffect(() => {
        const priStations = mapIdsToOptions(stationData?.nearbyStations, prevStations);
        setSelectedStations(priStations);
    }, [mapIdsToOptions, prevStations, stationData?.nearbyStations]);
...
export default EditStationData;
