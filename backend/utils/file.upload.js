...
async function uploadBufferFile(file, directory, fileName, section) {
  ...
  const fullPath = `${directory}/${fileName}$${
    section == 'document' ? '' : extension(file)
  }`;
...
async function deleteFileByUrl(fileUrl, section = 'all') {
  try {
    ...
    const secondLastPart = pathParts
      .slice(pathParts.length - 2, pathParts.length)
      .join('/');
    // Delete the file from Firebase Storage
    const part = secondLastPart.split('.');
    const dollarLink = `${part.slice(0, part.length - 1).join('.')}$.${
      part[part.length - 1]
    }`;

    await bucket
      .file(section == 'document' ? secondLastPart : dollarLink)
      .delete();
